      ...
      <div v-for="(section, idx) in sections" :key="section.title">
        ...
        >
          <div class="flex items-center gap-3 cursor-pointer">
            ...
            <span class="text-[#1D2026] font-medium text-base">{{ section.title }}</span>
          </div>
        ...
        <div v-if="openSection === idx && section.lectures && section.lectures.length" class="bg-white">
          <div class="px-6 py-2">
            <div v-for="(lecture, lectureIdx) in section.lectures" :key="lectureIdx" class="mb-2">
              <!-- Основная лекция -->
              <div class="flex items-center justify-between py-2" :class="lecture.active ? 'bg-[#E8F1FF] px-3 rounded' : ''">
                <div class="flex items-center">
                  <!-- Чекбокс -->
                  <div class="w-5 h-5 mr-3 border-2 rounded flex items-center justify-center" :class="lecture.completed ? 'bg-[#2467EC] border-[#2467EC]' : 'bg-white border-[#D1D5DB]'">
                  ...
                  <span :class="lecture.completed ? 'text-[#1D2026] font-semibold' : 'text-[#4E5566]'">{{ lecture.title }}</span>
                </div>
                <span class="text-[#8C94A3] text-sm">{{ lecture.duration }}</span>
              </div>
              
              <!-- Подлекции с отступом -->
              <div v-if="lecture.children && lecture.children.length" class="ml-8">
                <div v-for="(child, childIdx) in lecture.children" :key="childIdx" class="flex items-center justify-between py-2">
                  <div class="flex items-center">
                    <!-- Чекбокс для подлекции -->
                    <div class="w-5 h-5 mr-3 border rounded flex items-center justify-center" :class="child.completed ? 'bg-[#2467EC] border-[#2467EC]' : 'bg-white border-[#D1D5DB]'">
                      <svg v-if="child.completed" class="w-3 h-3 text-white" fill="none" viewBox="0 0 16 16">
                        <polyline points="3,8 7,12 13,4" fill="none" stroke="white" stroke-width="2"/>
                      </svg>
                    </div>
                    <span class="text-[#4E5566]">{{ child.title }}</span>
                  </div>
